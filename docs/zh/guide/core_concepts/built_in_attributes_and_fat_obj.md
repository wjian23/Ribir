...
Ribir 提供了一个强大的内置属性系统，让您可以为任何 Widget 添加常用功能，如布局控制（margin、alignment）、视觉效果（background、border、opacity、transform）和交互事件（on_tap、on_hover）。这些功能并非由每个 Widget 单独实现，而是通过一个名为 `FatObj` 来统一提供。
...
1.  **获取 Builder**: 调用 `Declare` trait 的 `declarer()` 方法来获取该部件的 Builder。
...
为了支持上述过程，Ribir 提供了 `#[derive(Declare)]` 宏，库中大部分的组件都是通过该宏来实现的。该宏会自动为您的 Widget 生成必要的代码，包括：
*   创建一个 **Builder**（例如 `Text` 的 `TextBuilder`）。
*   为您的 Widget 实现 **`Declare` trait**，将其与构建器关联。
*   为每个字段生成 **`with_xxx` 方法**，让您能够流畅地设置值。
*   为 Builder 实现 **`ObjDeclarer`**，处理最终的构建步骤 `finish()` 并返回 `FatObj<T>`（其中 `T` 是正在构建的 Widget 类型）。

**note**: Ribir 还提供了 `#[simple_declare]` 宏，该宏会为您的 Widget 生成一个简化的 Builder，该 Builder 最终将返回 `T`。适用于不需要内置属性或复杂状态管理的 Widget。


## 什么是 FatObj？

`FatObj<T>` 是 Ribir 核心库中的一个泛型结构体，其作用是在构建阶段临时包装一个 Widget，并为其附加各种内置属性，例如 `margin`、`background`、`on_tap` 等属性。


### 工作原理

1.  **惰性初始化**: `FatObj` 在内部维护所有内置属性（如 `margin`、`padding` 等）的状态，但它们默认为空。只有在您显式使用某个属性时，相关的组件才会被初始化。这确保了未使用的功能不会产生额外的性能开销。
...
`FatObj` 以固定顺序包装内置属性。这个顺序决定了最终 Widget 树的结构以及属性之间的相互作用方式。
...
9.  `margin`
10. `cursor`
11. **事件** (`mix_builtin`: `on_tap`, `on_pointer_move` 等)
12. `transform`
13. `opacity`
14. `visibility`
15. `h_align` / `v_align`
...
当您想为已构建的 Widget 添加内置属性时，可以直接使用 `@ FatObj { ... }` 语法，这更加简洁易读：
...
1. **`@ FatObj { ... }`**（最推荐）：直接用 FatObj 声明包装现有 Widget。这是最清晰、最地道的方法。

2. **`FatObj::new(w)` 然后 `@(w) { ... }`**：先创建一个 FatObj 变量，然后向其添加属性。

第一种方法（`@ FatObj { ... }`）通常更值得推荐，它更清晰、更简洁。

### 用 FatObj 包装声明的 Widget

使用 `#[derive(Declare)]` 宏定义的 Widget 在 DSL 中使用 `@` 语法声明时，会自动被 `FatObj` 包装，从而能够无缝使用内置属性。

`#[derive(Declare)]` 宏会生成一个实现了 `Declare` trait 的构建器，该 trait 包含一个 `FatObj<()>` 字段。当您在 DSL 中使用 `@` 时，它会自动用 `FatObj` 包装，以提供对内置属性的访问。

大多数情况下，Widget 都是用 `#[derive(Declare)]` 宏定义的，因此通常您可以直接使用 `@` 语法声明带有内置属性的 Widget。
```rust
use ribir::prelude::*;
...
内置属性（如 `opacity`、`background`、`margin`）是 `FatObj` 的属性。在声明式 UI 中，您通常在创建时将这些属性绑定到状态。然而，如果需要从代码中动态修改它们（例如在事件处理程序内），只要获取到对应的 `StateWriter` 就可以进行修改了。
...
fn dynamic_opacity_example() -> Widget<'static> {
    fn_widget! {
        ...
        // 在事件处理程序中修改 Widget 的属性
        // 1. 调用属性方法（例如 `.opacity()`）获取属性的写入器
        // 2. 获取对属性值的写入访问权
        @(w) {
            on_tap: move |_| {
...
在此示例中：
1.  `w.opacity()` 返回opacity属性的 `StateWriter`。
2.  该写入器上的 `$write(...)` 提供对实际 `f32` 值的可变访问。

此模式适用于所有内置属性（例如 `background()`、`margin()` 等）。请注意，您必须使用方法（例如 `opacity()`）而不是直接访问字段，因为字段是私有的。
