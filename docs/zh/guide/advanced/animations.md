...
3. **静默更新**: 动画期间的状态更改静默运行，这意味着它们不会触发可能导致无限循环的额外反应更新。这是通过动画系统对动画帧期间状态修改的特殊处理来实现的。

4. **状态传播**: 在动画期间，状态更改通过反应系统使用 `StateWriter` 的 `shallow()` 方法传播。此方法更新状态并通知 Widget 系统进行高效重绘，但它**不会**触发所有监听器（如 `pipe!` 块中）的完整反应通知，以避免性能开销和潜在的无限循环。

   这就是为什么将 `Animate` 绑定到**Widget 的写入器**（例如 `widget.map_writer(...)`）而非独立的 `Stateful` 变量至关重要的原因。当绑定到 Widget 的写入器时，`shallow()` 更新正确通知 Widget 使用新的插值重绘。
