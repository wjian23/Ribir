...
*   It implements **`ObjDeclarer`** for the builder, which handles the final build step `finish()` and returns `FatObj<T>` (where `T` is the type of the widget being built).
...
1.  **Lazy Initialization**: `FatObj` internally maintains the state of all built-in attributes (like `margin`, `padding`, etc.), but they default to empty. Only when you explicitly use an attribute is the related component initialized. This ensures that unused features don't bring additional performance overhead.
...
Here are the effects when multiple built-in attributes are set:
...
The first approach (`@ FatObj { ... }`) is generally preferred for its clarity and conciseness.
...
In most cases, widgets are defined with the `#[derive(Declare)]` macro, so normally you can use the `@` syntax to declare a widget with built-in attributes.
```rust
use ribir::prelude::*;
...
fn dynamic_opacity_example() -> Widget<'static> {
    fn_widget! {
        ...
        // 1. Call the property method (e.g., .opacity()) to get the property's writer
        // 2. Get write access to the property value
        @(w) {
            on_tap: move |_| {
