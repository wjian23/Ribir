...
widget_layout_test!(
  ...
  WidgetTester::new(fn_widget! {
    ...
    let box2 = @Container { width: *$read(box1.width()), height: *$read(box1.height()), margin: pipe!(*$read(box1.margin())) };
    @Row {
      @{ box1 }
      @{ box2 }
    }
...
fn simple_ref_bind_work() {
  ...
  let size = Size::new(100., 100.);
  let w = fn_widget! {
    let mut size2 = @Container { width: size.width, height: size.height };
    @Flex {
     @Container {
...
fn event_attr_sugar_work() {
  ...
  let w = fn_widget! {
    let mut sized_box = @Container { width: BEFORE_SIZE.width, height: BEFORE_SIZE.height };
    @(sized_box) {
      @Container {
        width: pipe!(*$read(sized_box.width())),
        height: pipe!(*$read(sized_box.height())),
        on_tap: move |_| {
          *$write(sized_box.width()) = AFTER_TAP_SIZE.width.into();
          *$write(sized_box.height()) = AFTER_TAP_SIZE.height.into();
        },
      }
    }
  };
...
fn expression_for_children() {
  ...
  let embed_expr = fn_widget! {
    let mut sized_box = @Container { width: size_one.width, height: size_one.height };
...
fn bind_fields() {
  ...
  let w = fn_widget! {
    ...
    let mut b = @Container { width: pipe!(*$read(a.width())), height: pipe!(*$read(a.height())) };
    let mut c = @Container { width: size.width, height: size.height };
    watch!(($read(a.width()).into_pixel(0.), $read(b.width()).into_pixel(0.)))
      .subscribe(move |(aw, bw)| {
        let w = aw + bw;
        *$write(c.width()) = w.into();
        *$write(c.height()) = w.into();
      });
    @Flex {
      on_tap: move |_| {
...
widget_layout_test!(
  ...
  WidgetTester::new(fn_widget! {
    let mut _sized = @Container { width: 1., height: 1. };
