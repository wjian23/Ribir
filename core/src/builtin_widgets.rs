...
impl<T> FatObj<T> {
  ...
  pub fn with_width<K: ?Sized>(&mut self, v: impl RInto<PipeValue<Dimension>, K>) -> &mut Self {
    init_sub_widget!(self, fixed_size, width, v)
  }
  ...
  pub fn with_height<K: ?Sized>(&mut self, v: impl RInto<PipeValue<Dimension>, K>) -> &mut Self {
    init_sub_widget!(self, fixed_size, height, v)
  }
  ...
  pub fn with_size<K: ?Sized>(&mut self, v: impl RInto<PipeValue<Size>, K>) -> &mut Self {
    let mix = self
      .mix_builtin
      .get_or_insert_with(MixBuiltin::default);
    let fixed_size = self
    ...
    mix.init_sub_widget(v, fixed_size, |w, v| {
      w.width = v.width.into();
      w.height = v.height.into();
    });
    self
  }
