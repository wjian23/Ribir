...
impl WrapRender for Anchor {
  fn place_children(&self, size: Size, host: &dyn Render, ctx: &mut LayoutCtx) {
...
impl<T: 'static> WrapRender for CustomAnchor<T> {
  fn place_children(&self, size: Size, host: &dyn Render, ctx: &mut LayoutCtx) {
    ...
    let mut pos = anchor.calculate(parent_size, size);

    // Anchor::calculate returns 0 for None components.
    // We should preserve existing position for None components if CustomAnchor
    // allows partial update? CustomAnchorFn returns `Anchor` struct. `Anchor`
    // struct has `x: Option<AnchorX>`, `y: Option<AnchorY>`. `calculate` method
    // treats `None` as 0. If we want to preserve, we must check manually.
